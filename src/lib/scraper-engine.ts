  ...
  try {
    ...
    if (paragraphs) {
      ...
      for (const p of paragraphs) {
        ...
        if (text.length > 40 && !/cookie|consent|newsletter|subscribe|privacy policy|terms of service|copyright|transition period|our partners|privacy settings|browser settings|personal data|Close.*Posts from this|added to your daily email digest|homepage feed|ShareGift/i.test(text)) {
          // Split paragraph into sentences
          const pSentences = text.split(/(?<=[.!?])\s+/);
          for (const s of pSentences) {
            const cleanS = s.trim();
            if (cleanS.length > 25 && !/subscribe|newsletter|follow us|sign up|read more|click here|written by|copyright|transition period|consent|cookie|our partners|privacy settings|browser settings|personal data|Close.*Posts from this|added to your daily email digest|homepage feed|ShareGift/i.test(cleanS)) {
